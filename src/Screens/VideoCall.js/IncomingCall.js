...
const isIOS = Platform.OS === 'ios';
  ...
  const showIncomingCallUi = ({handle, callUUID, name}) => {
   alert(handle, callUUID, name)
  }
  ...
  const answerCall = ({ callUUID }) => {
    ...
    //-------handle video=----//
     navigation.navigate('VideoCall',{number:'987654'})
  };
  ...
  const didReceiveStartCallAction = ({ handle }) => {
    ...
    navigation.navigate('VideoCall',{number: handle})
